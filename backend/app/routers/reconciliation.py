...
@router.post("/process", response_model=ReconciliationResponse, status_code=status.HTTP_200_OK)
def process_reconciliation(
    ...
    try:
        gateway_doc = gateway_col.find_one({"transaction_id": payload.transaction_id})
        bank_doc = bank_col.find_one({"transaction_id": payload.transaction_id})
    except PyMongoError as err:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    if not gateway_doc or not bank_doc:
        ...
        reason = "Gateway or bank transaction record is not yet available"
        ...
        reason = "Gateway payment successful and bank amount debited"
        ...
        reason = "Gateway payment failed but bank amount was debited"
        ...
        reason = "Gateway timed out but bank amount was debited"
    elif gateway_status == "TIMEOUT" and bank_status == "NOT_DEBITED":
        rec_status = ReconciliationStatus.PENDING.value
        action = ActionRequired.RETRY.value
        reason = "Gateway timed out and bank amount was not debited"
    elif gateway_status == "SUCCESS" and bank_status == "DELAYED":
        rec_status = ReconciliationStatus.PENDING.value
        action = ActionRequired.WAIT.value
        reason = "Gateway payment succeeded but bank transaction is delayed"
    else:
        rec_status = ReconciliationStatus.MISMATCH.value
        action = ActionRequired.INVESTIGATE.value
        reason = (
